...
function ModuleCard({ mod, index }) {
  ...
  return (
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      style={{ background: "var(--sm-surface)", border: `1.5px solid ${hov ? "rgba(21,101,192,.28)" : "var(--sm-border)"}`, borderRadius: 18, padding: "32px 26px", position: "relative", overflow: "hidden", cursor: "pointer", animation: `sosCardIn .5s ease ${0.04 + index * 0.08}s both`, transform: hov ? "translateY(-6px) scale(1.015)" : "translateY(0) scale(1)", boxShadow: hov ? "0 20px 48px rgba(21,101,192,.14), 0 0 0 3px rgba(21,101,192,.07)" : "none", transition: "transform .35s cubic-bezier(.22,.97,.47,1), box-shadow .35s ease, border-color .3s ease" }}
    >
      ...
      <div style={{ position: "absolute", top: 18, right: 18, width: 26, height: 26, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 800, background: hov ? "linear-gradient(135deg,#1565C0,#1DB88A)" : "var(--sm-surface-alt)", color: hov ? "#fff" : "var(--sm-text-muted)", border: hov ? "none" : "1px solid var(--sm-border)", transform: hov ? "scale(1.1)" : "scale(1)", boxShadow: hov ? "0 0 12px rgba(21,101,192,.35)" : "none", transition: "all .3s cubic-bezier(.22,.97,.47,1)" }}>{mod.n}</div>
      <div style={{ width: 60, height: 60, borderRadius: 18, background: hov ? "linear-gradient(135deg,rgba(21,101,192,.18),rgba(29,184,138,.14))" : "linear-gradient(135deg,rgba(21,101,192,.1),rgba(29,184,138,.08))", border: "1px solid rgba(21,101,192,.12)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28, marginBottom: 18, boxShadow: hov ? "0 6px 18px rgba(21,101,192,.2)" : "none", transition: "background .3s ease, box-shadow .3s ease" }}>{mod.icon}</div>
