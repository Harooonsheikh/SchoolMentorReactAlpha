...
function ModuleCard({ mod, index }) {
  ...
  return (
    <div
      ...
      style={{
        background: "var(--sm-surface)", border: `1.5px solid ${hov ? "rgba(21,101,192,.28)" : "var(--sm-border)"}`,
        borderRadius: 18, padding: "20px 16px", position: "relative", overflow: "hidden", cursor: "pointer",
...
export default function Tab1_MobileApp() {
  return (
    <>
      <style>{`
        ...
        <div className="sos-modules-padding" style={{ background: "var(--sm-surface-alt)", display: "flex", flexDirection: "column" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 24, flexWrap: "wrap" }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "rgba(21,101,192,.09)", border: "1px solid rgba(21,101,192,.18)", borderRadius: 100, padding: "5px 14px", flexShrink: 0, fontSize: 10, fontWeight: 700, letterSpacing: "1.5px", textTransform: "uppercase", color: "var(--sm-navy)" }}>
              <div style={{ width: 6, height: 6, borderRadius: "50%", background: "var(--sm-navy)", animation: "sosDotPulse 2s ease-in-out infinite" }} />{TAB.secPill}
            </div>
            <div style={{ fontSize: 20, fontWeight: 800, color: "var(--sm-text)" }}>
              {TAB.secH2[0]}<span style={{ background: "linear-gradient(90deg,#1565C0,#1DB88A)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>{TAB.secH2[1]}</span>{TAB.secH2[2]}
            </div>
