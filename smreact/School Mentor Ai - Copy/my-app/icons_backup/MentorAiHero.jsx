...
const AI_FEATURES = [
  { icon: "💬", label: "AI Chat",       sub: "Instant answers",     iconBg: "rgba(59,130,246,.25)"  },
  { icon: "📝", label: "Lesson Plans",  sub: "Cambridge & Bloom's", iconBg: "rgba(29,184,138,.25)"  },
  { icon: "📋", label: "AI Worksheets", sub: "Auto-generated",      iconBg: "rgba(167,139,250,.25)" },
  { icon: "🎨", label: "Design Studio", sub: "Social media posts",  iconBg: "rgba(251,191,36,.2)"   },
];

const FEATURES = ["AI Chat", "Lesson Plans", "AI Worksheets", "Design Studio"];

// ── Typing dots ───────────────────────────────────────────────────────────────
function TypingDots() {
  return (
    <div style={{ display:"flex", alignItems:"center", gap:8, marginTop:14, paddingTop:12, borderTop:"1px solid rgba(255,255,255,.1)" }}>
      <div style={{ display:"flex", gap:4 }}>
        {[0, 0.2, 0.4].map((delay, i) => (
          <div key={i} style={{
            width:5, height:5, borderRadius:"50%",
            background:"rgba(255,255,255,.6)",
            animation: `aiTypeDot 1.2s ${delay}s ease-in-out infinite`,
          }} />
        ))}
      </div>
      <span style={{ fontSize:11, color:"rgba(255,255,255,.55)", fontWeight:500 }}>
...
// ── Main export ───────────────────────────────────────────────────────────────
export default function MentorAiHero() {
  return (
    <>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&display=swap');
        @keyframes aiFadeUp   { from{opacity:0;transform:translateY(22px)} to{opacity:1;transform:translateY(0)} }
        @keyframes aiSlideIn  { from{opacity:0;transform:translateY(-50%) translateX(30px)} to{opacity:1;transform:translateY(-50%) translateX(0)} }
        @keyframes aiShimmer  { 0%{background-position:-200% 0} 100%{background-position:200% 0} }
        @keyframes aiPulse    { 0%{transform:scale(1);opacity:1} 100%{transform:scale(2.5);opacity:0} }
        @keyframes aiSpinGlow { 0%,100%{box-shadow:0 0 0 0 rgba(29,184,138,0)} 50%{box-shadow:0 0 12px 3px rgba(29,184,138,.5)} }
        @keyframes aiTypeDot  { 0%,80%,100%{transform:scale(0.6);opacity:.4} 40%{transform:scale(1);opacity:1} }
      `}</style>
      ...
      <section style={{
        ...
        <div style={{ position:"relative", zIndex:2, maxWidth:620 }}>
          ...
          <div style={{
            ...
            marginBottom:24,
            animation:"aiFadeUp .6s ease .1s both",
          }}>
            <div style={{
              ...
              fontSize:14,
              animation:"aiSpinGlow 4s linear infinite",
            }}>✦</div>
          ...
          <h1 style={{
            ...
            animation:"aiFadeUp .65s ease .2s both",
          }}>
            Smart Teaching Assistant<br />
            <span style={{
          ...
          <p style={{
            ...
            animation:"aiFadeUp .65s ease .3s both",
          }}>
            Mentor AI acts as a powerful assistant for teachers and principals, helping them create academic content instantly using intelligent automation.
          </p>

          {/* CTA */}
          <div style={{ animation:"aiFadeUp .65s ease .4s both" }}>
          ...
          <div style={{
            ...
            animation:"aiFadeUp .65s ease .5s both",
          }}>
            {FEATURES.map((f, i) => (
        ...
        <div style={{
          ...
          width:226, zIndex:2,
          animation:"aiSlideIn .7s ease .5s both",
        }}>
          ...
          {AI_FEATURES.map((f, i) => (
            <div key={i} style={{
              ...
              padding:"8px 0",
              borderBottom: i < AI_FEATURES.length - 1 ? "1px solid rgba(255,255,255,.1)" : "none",
            }}>
              <div style={{
              ...
              <div style={{ flex:1 }}>
                ...
                <div style={{ fontSize:10, color:"rgba(255,255,255,.5)", marginTop:1 }}>{f.sub}</div>
              </div>
              <div style={{
