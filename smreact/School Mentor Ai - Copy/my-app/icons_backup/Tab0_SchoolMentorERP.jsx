...
function ModuleCard({ mod, index }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ width: 46, height: 46, borderRadius: 13, background: hov ? "linear-gradient(135deg,rgba(21,101,192,.18),rgba(29,184,138,.14))" : "linear-gradient(135deg,rgba(21,101,192,.1),rgba(29,184,138,.08))", border: "1px solid rgba(21,101,192,.12)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 21, marginBottom: 12, boxShadow: hov ? "0 6px 18px rgba(21,101,192,.2)" : "none", transition: "background .3s ease, box-shadow .3s ease" }}>{mod.icon}</div>
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @media (min-width: 1536px) {
          .sos-modules-grid { grid-template-columns: repeat(6,1fr) !important; }
        }
      `}</style>
      ...
      <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", display: "flex", flexDirection: "column", minHeight: "100%", background: "var(--sm-surface-alt)" }}>
          ...
          <div className="sos-hero-grid" style={{ position: "relative", zIndex: 2 }}>
            <div>
              ...
              <h1 style={{ fontSize: "clamp(28px,3.5vw,46px)", fontWeight: 900, color: "#fff", lineHeight: 1.05, letterSpacing: -1.5, marginBottom: 12, animation: "sosFadeUp .55s ease .12s both" }}>
                ...
                <sup style={{ fontSize: 14, verticalAlign: "super", WebkitTextFillColor: "rgba(255,255,255,.5)", fontWeight: 400 }}>™</sup>
              ...
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap", animation: "sosFadeUp .55s ease .36s both" }}>
                {TAB.stats.map((s, i) => (
                  <div key={i} style={{ background: "rgba(255,255,255,.12)", border: "1px solid rgba(255,255,255,.18)", borderRadius: 12, padding: "10px 16px", cursor: "default", transition: "all .3s ease" }}
            ...
            <div className="sos-float-col" style={{ animation: "sosFadeUp .55s ease .2s both" }}>
              {TAB.floatCards.map((c, i) => (
                <div key={i} style={{ background: "rgba(255,255,255,.14)", border: "1px solid rgba(255,255,255,.22)", borderRadius: 14, padding: "14px 16px", backdropFilter: "blur(8px)", cursor: "default", animation: `sosFadeUp .5s ease ${.25 + i * .12}s both`, transition: "all .3s ease" }}
                  onMouseEnter={e => { e.currentTarget.style.boxShadow = "0 12px 32px rgba(0,0,0,.18)"; e.currentTarget.style.background = "rgba(255,255,255,.22)"; }}
                ...
                >
                  ...
                  <div style={{ fontSize: 22, fontWeight: 900, color: "#fff", lineHeight: 1 }}>{c.v}</div>
                  <div style={{ fontSize: 9.5, color: "rgba(255,255,255,.6)", fontWeight: 600, textTransform: "uppercase", letterSpacing: ".8px", marginTop: 2 }}>{c.l}</div>
        ...
        <div className="sos-modules-padding" style={{ background: "var(--sm-surface-alt)", display: "flex", flexDirection: "column" }}>
          ...
          <div className="sos-modules-grid">
        ...
        <div className="sos-cta-padding sos-cta-row" style={{ background: "linear-gradient(135deg,#1565C0 0%,#1178a8 50%,#1DB88A 100%)", position: "relative", overflow: "hidden", flexShrink: 0 }}>
          ...
          <div className="sos-cta-btns">
            ...
            <CtaBtn onClick={() => { window.open("https://wa.me/923700036867?text=" + encodeURIComponent("Hi, I'd like to talk to an expert about School Mentor."), "_blank"); }}>💬 Talk to an Expert</CtaBtn>
          </div>
        </div>
      </div>
    </>
