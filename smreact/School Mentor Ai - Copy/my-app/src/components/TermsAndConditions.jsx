  ...
  },
  {
    ...
    body: (
      <>
        ...
        <Highlight icon={<i className="fa-solid fa-lock" />} text="Users must not share accounts without proper authorization. School Mentor is not liable for unauthorized access resulting from a school's failure to manage credentials appropriately." />
      </>
    ),
  },
  {
  ...
  },
  {
    ...
    body: (
      <>
        ...
        <Highlight icon={<i className="fa-solid fa-triangle-exclamation" aria-hidden="true" style={{ color: "var(--sm-amber)" }} />} text="Always review AI-generated content for accuracy, appropriateness, and curriculum alignment before publishing or distributing it." />
      </>
    ),
  },
  {
  ...
  },
  {
    ...
    body: (
      <>
        ...
        <Highlight icon={<i className="fa-solid fa-comment-dots" />} text="If you have any questions about these Terms and Conditions or need clarification on any point, our team is happy to help. We typically respond within 3 business days." />
      </>
    ),
  },
];
...
// ── Responsive hook (matches Privacy Policy / LegalPages) ─────────
  ...
  return (
    <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", background: "var(--sm-bg)", color: "var(--sm-text)", minHeight: "100vh" }}>
      ...
      <div style={{
        ...
        padding: isMobile ? "40px 16px 50px" : "60px 44px 68px", position: "relative", overflow: "hidden",
      }}>
        {/* Grid */}
        ...
        <div style={{ position: "relative", zIndex: 2, textAlign: "center", maxWidth: 700, margin: "0 auto" }}>
          ...
          <p style={{ fontSize: isMobile ? 14 : 15, color: "rgba(255,255,255,.78)", lineHeight: 1.75, maxWidth: 600, margin: "0 auto", animation: "tcFadeUp .6s ease .2s both" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, marginTop: 22, flexWrap: "wrap", animation: "tcFadeUp .6s ease .28s both" }}>
            {["May 2026", "School Mentor®", "Legally Binding"].map((chip, i) => (
              <div key={i} style={{ display: "inline-flex", alignItems: "center", gap: 7, background: "rgba(255,255,255,.13)", border: "1px solid rgba(255,255,255,.22)", borderRadius: 100, padding: "5px 13px", fontSize: isMobile ? 11 : 12, fontWeight: 600, color: "rgba(255,255,255,.88)" }}>
                {chip}
              </div>
            ))}
          </div>
      ...
      {/* ── Page body ── */}
      <div style={{
        display: "grid", gridTemplateColumns: isTablet ? "1fr" : "260px 1fr",
        gap: isTablet ? 0 : 28, maxWidth: "clamp(1200px, 93vw, 1850px)", margin: "0 auto",
        padding: isMobile ? "20px 14px 48px" : isTablet ? "24px 20px 56px" : "32px 24px 60px", alignItems: "start",
      }}>

        {/* TOC Sidebar */}
        <aside style={{
          // See LegalPages.jsx — without this the panel's min-content width
          // keeps it from shrinking to fit a narrow phone.
          minWidth: 0,
          ...(isTablet ? {
            display: "grid",
          ...
          <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: 2, textTransform: "uppercase", color: "var(--sm-text-muted)", paddingBottom: 12, borderBottom: "1px solid var(--sm-border)", marginBottom: 10, ...(isTablet ? { gridColumn: "1 / -1", marginBottom: 6 } : {}) }}>
            Table of Contents
          </div>
          {TOC.map((item) => {
        ...
        <div style={{ minWidth: 0, animation: "tcFadeUp .6s ease .4s both" }}>
          ...
          <div style={{
            ...
            <i className="fa-solid fa-circle-check" aria-hidden="true" /> Last Updated: May 2026
          </div>
          ...
          <div style={{
            ...
            <span style={{ fontSize: 20, flexShrink: 0, marginTop: 2, color: "var(--sm-navy)" }}><i className="fa-solid fa-clipboard-list" aria-hidden="true" /></span>
      ...
      {/* ── Footer ── */}
      <div style={{
        background: "#0d1b2e", padding: isMobile ? "18px 16px" : "22px 40px",
