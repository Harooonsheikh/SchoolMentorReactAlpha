 ...
 *
 * Agent/super-admin only on the server — which is exactly who this console is,
 * so it goes up WITH the bearer token (the school-side copy of this file has to
 * strip it: a token carrying `BranchID` is refused here with a bare 403).
 * `closedByAgentID` records who pressed the button — sessionStorage
 * `superadminid`, the same id /support/agents is keyed by.
 */
