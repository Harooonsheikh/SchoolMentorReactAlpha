...
export function useSupportChat({
  ...
  /* Koi apna message abhi tak Read nahi hua? Tab tak poora sync har tick par
     karo taake tick foran blue ho — warna sasta check hi kaafi hai. */
  ...
  /* "Dekh liya" ke liye chat ka khula hona kaafi nahi — window par nazar bhi
     honi chahiye. Do window saath saath khuli hon (ERP ek taraf, console
     doosri taraf) to sirf `viewing` dekhne par aate hi message read ho jata
     tha aur bhejne wale ko foran blue tick mil jata tha, halanke doosri taraf
     wale ne dekha tak nahi hota tha. Tab chhupi ho ya window focus me na ho to
     ab sirf "delivered" hota hai; focus wapas aate hi read. */
  ...
  /* Server ke taza rows dekh kar apne bubbles ke ticks aage barhao.
     Sirf wo ids bhejte hain jinka status waqai barha ho — warna har sync par
     be-wajah re-render hota. */
  const syncOutTicks = useCallback((id, rows) => {
    ...
    (rows || []).forEach((m) => {
      ...
      const status = Number(m.messageStatus) || MessageStatus.Sent;
      if (status < MessageStatus.Read) pending = true;
      const prev = outStatusRef.current.get(m.messageId) || 0;
      if (status <= prev) return;
      outStatusRef.current.set(m.messageId, status);
      if (status >= MessageStatus.Read) read.push(m.messageId);
      else if (status === MessageStatus.Delivered) delivered.push(m.messageId);
    });
  ...
  const openSession = useCallback(async (id) => {
    ...
    /* History apne saath sahi statuses laati hai — unhi ko baseline maan lo,
       warna pehle sync par har purana message dobara "receipt" ban jata. */
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        /* Apne bubbles ke ticks bhi yahin taza kar do. Widget band hone ke baad
           dobara kholne par `start()` chalta hi nahi (startedRef), yani
           transcript reload nahi hota — is liye jo message darmiyan me read ho
           chuka hota, uska tick purana (single/grey) hi para rehta tha. */
    ...
    const tick = async () => {
      try {
        ...
        /* Sasta check: sessions list ek chhoti si row deti hai. Jab tak
           ...
           Magar apna koi message abhi Read na hua ho to sasta check kaafi nahi:
           status badalne se lastMessageAt nahi badalta, is liye us soorat me
           har tick par poora sync — tick foran double/blue ho jaye. */
        ...
        /* Apne bubbles ke ticks — ye "fresh" messages se pehle, kyunki purane
           messages ka status badalna bhi utna hi zaroori hai. */
        syncOutTicks(sessionId, detail.messages);
        ...
        if (detail.sessionStatus === SessionStatus.Closed) {
          ...
          /* Band hone ka elaan SIRF EK BAAR. Pehle ye har tick par chalta tha
             (session to band hi rehti hai), is liye har 2 second par ek naya
             "session closed" message screen par lag jata tha aur poll bhi
             be-wajah chalta rehta tha. */
  ...
  const sendText = useCallback(async (body) => {
    ...
    if (result.message?.messageId != null) {
      ...
      /* Abhi bheja hai → hamesha "Sent". Send ke response ka status par
         bharosa nahi: kabhi wo pehle se Read (3) bhej deta hai, jis se apna
