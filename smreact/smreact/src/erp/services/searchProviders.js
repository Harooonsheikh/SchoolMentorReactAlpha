...
const loadStudents = lazyMock(() => import('../mock/students'));
const loadHr       = lazyMock(() => import('../mock/hr'));
const loadLP       = lazyMock(() => import('../mock/lessonPlans'));
const loadExams    = lazyMock(() => import('../mock/exams'));
const loadTT       = lazyMock(() => import('../mock/timetable'));
const loadFee      = lazyMock(() => import('../mock/fee'));
const loadAccounts = lazyMock(() => import('../mock/accounts'));
...
const studentsProvider = {
  ...
  async search(query, ctx) {
    const m = await loadStudents();
    const classes = m.mockStuClasses || [];
    const out = [];
...
const hrProvider = {
  ...
  priority:    90,
  async search(query, ctx) {
    const m = await loadHr();
    const out = [];
    /* Employees */
    for (const e of (m.mockHrEmployees || [])) {
      ...
      if (matchAny(
        ...
        out.push({
          ...
          title:    full,
          subtitle: `${e.eid || ''}${e.fn ? ' · ' + e.fn : ''}`,
          preview:  [e.phone && `Phone: ${e.phone}`, e.email].filter(Boolean).join('  ·  '),
    ...
    for (const d of (m.mockHrDesigs || [])) {
      if (matchAny(query, d.name)) {
        out.push({
          id:       `hr-desig-${d.id}`,
          title:    d.name,
...
const attendanceProvider = {
  ...
  priority:    55,
  async search(query, ctx) {
    const m = await loadTT();
    const out = [];
    for (const c of (m.mockTimeTableClasses || [])) {
      if (matchAny(query, c.cls, c.sec, `${c.cls} ${c.sec}`)) {
        out.push({
          id:       `att-${c.cls}-${c.sec}`,
          title:    `${c.cls} ${c.sec}`,
          subtitle: 'Class attendance',
          path:     ['Attendance', `${c.cls} ${c.sec}`],
        });
        if (out.length >= ctx.limit) return out;
      }
    }
...
const feeProvider = {
  ...
  priority:    45,
  async search(query, ctx) {
    const m = await loadFee();
    const out = [];
    for (const c of (m.mockFeeClasses || [])) {
      if (matchAny(query, c.cls, c.name, c.label)) {
        out.push({
          id:       `fee-class-${c.cls || c.name}`,
          title:    c.name || c.cls,
    ...
    for (const f of (m.mockFamilies || [])) {
      if (matchAny(query, f.familyName, f.father, f.mother, f.phone, f.cnic, f.fId)) {
        out.push({
          id:       `fee-fam-${f.fId}`,
          title:    f.familyName || f.father || `Family ${f.fId}`,
          subtitle: `Family · ${f.fId || ''}`,
          preview:  f.phone ? `Phone: ${f.phone}` : '',
...
const accountsProvider = {
  ...
  priority:    40,
  async search(query, ctx) {
    const m = await loadAccounts();
    const out = [];
    for (const t of (m.mockAccTypes || [])) {
      if (matchAny(query, t.name, t.code)) {
        out.push({
          id:       `acc-type-${t.id || t.name}`,
          title:    t.name,
          subtitle: 'Account head',
          preview:  t.code ? `Code: ${t.code}` : '',
          path:     ['Accounts', 'Heads'],
        });
        if (out.length >= ctx.limit) return out;
      }
    }
    /* Voucher rows — keyed by date string in mockAccTxns. */
    const txns = m.mockAccTxns || {};
    for (const dateKey of Object.keys(txns)) {
      for (const v of (txns[dateKey] || [])) {
        if (matchAny(query, v.head, v.narr, v.voucher, v.party, v.ref)) {
          out.push({
            id:       `acc-vch-${v.voucher || dateKey + '-' + (v.id || v.head)}`,
            title:    v.voucher || v.head || 'Voucher',
            subtitle: [v.head, v.party].filter(Boolean).join(' · '),
            preview:  v.narr || '',
...
const userPermissionsProvider = {
  ...
  async search(query, ctx) {
    ...
    const roles = mod.MOCK_ROLES || mod.ROLES || [];
    for (const r of roles) {
      if (matchAny(query, r.name, r.description, r.scope)) {
        out.push({
          id:       `perm-r-${r.id || r.name}`,
          title:    r.name,
          subtitle: 'Role',
