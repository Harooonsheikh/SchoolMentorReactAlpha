...
function TTDownloadModal({
  ...
  const filtersContent = (
    ...
    >
      ...
      <button
        type="button"
        onClick={() => setReportType('periodcount')}
        style={{
          padding: '10px 8px',
          borderRadius: 10,
          cursor: 'pointer',
  ...
  const generate = async ({ style, format }) => {
    ...
    if (reportType === 'daily') {
      ...
      title =
        `Daily Timetable — ${target.cls} · ${target.section} · ${DAYS[day]}`;
    }

    else if (reportType === 'weekly') {
      const wp = {};

      weekPeriods.forEach((p, di) => {
        wp[di] = p;
      });

      html = buildWeeklyTTReport({
        header,
        cls: target.cls,
        section: target.section,
        weekPeriods: wp,
      ...
      title =
        `Weekly Timetable — ${target.cls} · ${target.section}`;
    }
      ...
      title =
        `Period Count — ${target.cls} · ${target.section}`;
    }
