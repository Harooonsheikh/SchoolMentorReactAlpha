  ...
  return `
    <div class="r-head-wrap">
      <div class="r-brand-row">
        <div class="r-logo" style="color:${brandColor};border-color:${brandColor}">${SCHOOL_LOGO_SVG}</div>
        <div class="r-brand-info">
          <div class="r-school-name" style="color:${brandColor}">${SCHOOL.name}</div>
      ...
      <div class="r-meta-bar">
        <span><strong>Issue Date:</strong> ${ctx.fmtDate(new Date().toISOString().slice(0,10))}</span>
        ${extraMeta?`<span>${extraMeta}</span>`:''}
        <span><strong>${SCHOOL.tagline}</strong></span>
...
function reportFooter(C, note, ctx) {
  return `<div class="r-footer">${note||''}<br>Generated by <strong>School Mentor ERP</strong> &middot; ${ctx.fmtDate(new Date().toISOString().slice(0,10))} &middot; All values in PKR unless stated otherwise.</div>`;
}
...
export function generateLoanReportHTML(e, style, ctx) {
  ...
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Loan / Advance Report — ${getFullName(e)}</title><link href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;600;700;800&display=swap" rel="stylesheet"><style>${reportBaseCSS(C, style, 'portrait')}</style></head><body>
    <div class="page">
      ...
      ${reportSignatures(C)}
      ${reportFooter(C, `This Loan / Advance Account Statement reflects every loan disbursement and repayment recorded for <strong>${getFullName(e)}</strong> in the School Mentor ERP up to <strong>${fmtDate(new Date().toISOString().slice(0,10))}</strong>.`, ctx)}
    </div>
    ${CLOSE_SCRIPT_AFTER_PRINT}
  </body></html>`;
}
...
function hrBuildReportHTML(fileTitle, title, filtersHtml, innerHtml) {
  const sn = SCHOOL.name;
  return `<!DOCTYPE html><html><head><meta charset="utf-8"><title>${fileTitle}</title>
    ...
    <div class="rep-page">
      <div class="rep-head">
        <div class="rep-logo">${HR_LOGO_SVG}</div>
        <div><div class="rep-name">${sn}</div><div class="rep-title">${title}</div></div>
      </div>
...
export function generateHrDirectoryReport(ctx) {
  ...
  const filters = `<span><b>Total Staff:</b> ${emps.length}</span><span><b>Active:</b> ${active}</span><span><b>Inactive:</b> ${emps.length-active}</span><span><b>Departments:</b> ${depts.length}</span><span><b>Generated:</b> ${hrToday()}</span>`;
  ...
  return hrBuildReportHTML('Employee Directory', 'Human Resource — Employee Directory', filters, inner);
}
...
export function generateHrSalaryRegister(ctx, monthKey) {
  ...
  const filters = `<span><b>Period:</b> ${monthName} ${yearVal}</span><span><b>Employees:</b> ${activeEmps.length}</span><span><b>Gross:</b> PKR ${hrFmtMoney(gGross)}</span><span><b>Net Payable:</b> PKR ${hrFmtMoney(gNet)}</span><span><b>Generated:</b> ${hrToday()}</span>`;
  ...
  return hrBuildReportHTML(`Salary Register — ${monthName} ${yearVal}`, `Human Resource — Salary Register · ${monthName} ${yearVal}`, filters, inner);
}
...
export function generateHrLoanSummary(ctx) {
  ...
  const filters = `<span><b>With Loans:</b> ${empWithLoans}</span><span><b>Total Issued:</b> PKR ${hrFmtMoney(gIssued)}</span><span><b>Recovered:</b> PKR ${hrFmtMoney(gDeducted)}</span><span><b>Outstanding:</b> PKR ${hrFmtMoney(gOutstanding)}</span><span><b>Generated:</b> ${hrToday()}</span>`;
  ...
  return hrBuildReportHTML('Loan & Advance Ledger', 'Human Resource — Loan & Advance Ledger', filters, inner);
}
...
export function generateHrDeptSummary(ctx) {
  ...
  const filters = `<span><b>Departments:</b> ${depts.length}</span><span><b>Total Headcount:</b> ${gHead}</span><span><b>Monthly Cost:</b> PKR ${hrFmtMoney(gCost)}</span><span><b>Generated:</b> ${hrToday()}</span>`;
  ...
  return hrBuildReportHTML('Department Summary', 'Human Resource — Department Summary Report', filters, inner);
}
...
export function generateHrLeaveRegister(ctx) {
  ...
  const filters = `<span><b>Active Employees:</b> ${activeEmps.length}</span><span><b>Deduction Enabled:</b> ${deductCount}</span><span><b>Generated:</b> ${hrToday()}</span>`;
  ...
  return hrBuildReportHTML('Leave & Attendance Register', 'Human Resource — Leave & Attendance Register', filters, inner);
}
...
export function generateHrPayrollSummary(ctx, monthKey) {
  ...
  const filters = `<span><b>Period:</b> ${monthName} ${yearVal}</span><span><b>Total Employees:</b> ${activeEmps.length}</span><span><b>Paid:</b> ${cPaid}</span><span><b>Pending:</b> ${cGen+cNot}</span><span><b>Net Payable:</b> PKR ${hrFmtMoney(gNet)}</span><span><b>Generated:</b> ${hrToday()}</span>`;
  const inner = `
  ...
  return hrBuildReportHTML(`Payroll Summary — ${monthName} ${yearVal}`, `Human Resource — Payroll Summary · ${monthName} ${yearVal}`, filters, inner);
}
